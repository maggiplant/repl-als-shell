...
;; 	(progn (print entire-stream) (read-from-string entire-stream 'nil :eof))
...
;; (defun read-entire-stream (stream)
;;   (let ((stream-as-string 'nil)
;; 	(char 'nil))
;;     (loop 
;; 	  when (or (equal char #\newline) (equal char :eof))
;; 	    do (progn (setq char (read-char stream 'nil :eof t))
;; 		      (concatenate 'string stream-as-string (string char)))
;; 	  do (return stream-as-string))))
...
;; (defun read-entire-stream (stream &optional partial-bla stream-as-string)
;;   (let ((char (read-char stream 'nil :eof t))) ;; een teken ophalen en opslaan als char
;;     (cond ((and (or (equal char #\newline) (equal char :eof)) (stringp stream-as-string)) stream-as-string) ;; Als stream-as-string een string is en het opgehaalde teken is een newline of eof, dan stream-as-string teruggeven
;; 	  ((not (stringp stream-as-string)) "")
;; 	  (t (read-entire-stream stream )) ;; Als dit niet het geval is, recursief aanroepen
;; 	   )))
